...
{
public:
    netAddr(const std::string &ip, uint16_t port) : _ip(ip), _port(port) // for client,host to net
    {
        _addr.sin_family = AF_INET;
        _addr.sin_addr.s_addr = ::inet_addr(_ip.c_str());
        _addr.sin_port = htons(_port);
    }
    netAddr(const sockaddr_in &addr) : _addr(addr) // for client,net to host
    {
        ipToHost();
        portToHost();
    }
    netAddr(uint16_t port) : _ip("all ips of this host"), _port(port) // for server,host to net
    {
        _addr.sin_family = AF_INET;
